...
function checkOrientation() {
    ...
    // ✅ MODIFIED: Allow both portrait and landscape modes
    // Always hide the orientation lock and start the website
    isLandscape = true; // Set to true to enable all features
    ...
    // Force resize matrix for proper display
    setTimeout(() => {
        forceResizeMatrix();
    }, 100);
    ...
    if (isMobile) {
        ...
        mediaQuery.addEventListener('change', (e) => {
            // Just resize the matrix, don't stop the website
            setTimeout(() => {
                forceResizeMatrix();
            }, 100);
        });
                    ...
                    if (currentSettings.enableBook === true) {
                        if (canvas && giftImage && matrixCanvas) {
                            ...
                            if (giftImage.src && giftImage.src !== window.location.href && giftImage.src !== '' && !giftImage.src.includes('undefined')) {
                                // Force display and ensure image is loaded
